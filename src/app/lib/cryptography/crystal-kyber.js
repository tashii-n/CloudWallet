  ...
  for (let i = 0; i < paramsK; i++) {
    ...
    for (let j = 0; j < paramsK; j++) {
      ...
      const output = new Uint8Array(
        xof.digest({ buffer: Buffer.alloc(672), format: "binary" })
      );
...
function prf(l, key, nonce) {
  ...
  const buf = Buffer.from(
    hash.digest({ buffer: Buffer.alloc(l), format: "hex" }),
    "hex"
  );
  // Its commented after issue
...
function polyBaseMulMontgomery(a, b) {
  ...
  for (let i = 0; i < paramsN / 4; i++) {
    ...
    [a[4 * i + 0], a[4 * i + 1], a[4 * i + 2], a[4 * i + 3]] = [
      rx[0],
      rx[1],
      ry[0],
      ry[1],
...
// 1. KeyGen
export async function keyGen768(seed) {
  // IND-CPA keypair
  ...
  for (let i = 0; i < paramsK; i++) {
    for (let j = 0; j < paramsN / 4; j++) {
      for (let k = 0; 4 > k; k++) {
        ...
        t[k] =
          (((u[i][4 * j + k] << 10) + paramsQ / 2) / paramsQ) & 0b1111111111;
      }
...
/*****************************************************************************************************************************/
// 2. Encrypt
export async function Encrypt768(pk, m) {
